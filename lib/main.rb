...
class Game
    ...
    attr_reader :amount_of_mistakes, :used_letters, :answer

    def initialize()
        @amount_of_mistakes = 0
        @used_letters = []
        @random_word = ""
        @random_word = File.readlines("google-10000-english-usa-no-swears.txt")[rand(0..9870)].chomp until 4 < @random_word.length && @random_word.length < 13 
